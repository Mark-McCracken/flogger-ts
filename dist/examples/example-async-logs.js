...
let generatePathLocation = (suffix) => {
    if (suffix === "log")
        return path.resolve(`${__dirname}/volume/logs/async-logs/${currentDate}.log`);
...
flogger_ts_1.redirectLoggingToFiles(loggingConfig);
let reusableLogLocation = path.resolve(`${__dirname}/../../../volume/logs/async-logs/reusable_logs`);
let reusableLogger = new flogger_ts_1.ReusableLogger(reusableLogLocation);
reusableLogger.log({
...
console['logWithColor']([flogger_ts_1.Colors.FgMagenta], "red stuff");
...
flogger_ts_1.removeEmptyLogFiles(loggingConfig);
