...
const flogger_ts_1 = require("flogger-ts");
let currentDate = flogger_ts_1.currentDateString();
let generatePathLocation = (suffix) => {
    if (suffix === "log")
        return path.resolve(`${__dirname}/volume/logs/sync-logs/${currentDate}.log`);
...
flogger_ts_1.redirectLoggingToFilesSync(loggingConfig);
...
let reusableLogger = flogger_ts_1.createReusableLoggerSync(reusableLogLocation);
let item1 = {
...
try {
    ...
    if (err.contains(`ENOENT`)) {
        ...
        flogger_ts_1.removeEmptyLogFilesSync(loggingConfig);
...
console['logWithColor']([flogger_ts_1.Colors.FgGreen], "stuff to colorfully log");
let files;
try {
        ...
        files.forEach(logFile => {
            ...
            try {
                ...
                if (contents) {
                    ...
                    console.info(`${flogger_ts_1.currentTimestampString()}`);
